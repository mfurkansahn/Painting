...
public class PaintableCanvas : MonoBehaviour
{
    ...
    [SerializeField] UnityEvent<float, bool> OnSyncUIWithBrushStrength = new();
    [SerializeField] UnityEvent<float, bool> OnSyncUIWithBrushScale = new();
    ...
    float? ActiveBrushStrength;
    float? ActiveBrushScale;
    ...
    public void SetBrushScale(float InScale)
    {
        AppendCommand(new PaintingCommand_SetBrushScale(InScale, ActiveBrushScale != null));
    }

    public void SetBrushStrength(float InStrength)
    {
        AppendCommand(new PaintingCommand_SetBrushStrength(InStrength, ActiveBrushStrength != null));
    }
    ...
    void ExecuteCommand(BasePaintingCommand InCommand, bool IsUndoOrReplay)
    {
        ...
        else if (InCommand is PaintingCommand_SetBrush)
        {
            ...
            int ScaledBrushWidth = Mathf.RoundToInt(ActiveBrush.BrushTexture.width * ActiveBrushScale.Value);
            int ScaledBrushHeight = Mathf.RoundToInt(ActiveBrush.BrushTexture.height * ActiveBrushScale.Value);
        ...
        else if (InCommand is PaintingCommand_SetBrushScale)
        {
            (InCommand as PaintingCommand_SetBrushScale).Execute(ref ActiveBrushScale);

            if (IsUndoOrReplay)
                OnSyncUIWithBrushScale.Invoke(ActiveBrushScale.Value, false);
        ...
        else if (InCommand is PaintingCommand_SetBrushStrength)
        {
            (InCommand as PaintingCommand_SetBrushStrength).Execute(ref ActiveBrushStrength);

            if (IsUndoOrReplay)
                OnSyncUIWithBrushStrength.Invoke(ActiveBrushStrength.Value, false);
        }
    }
        ...
        InDrawCommand.Execute(InIsExtension, (Vector2 InLocation) =>
        {
            ...
            for (int BrushY = 0; BrushY < ActiveBrush.Height; BrushY++)
            {
                ...
                for (int BrushX = 0; BrushX < ActiveBrush.Width; BrushX++)
                {
                    ...
                    CanvasPixel = ActiveBrush.Apply(CanvasPixel, BrushPixel, ActiveColour.Value, ActiveBrushStrength.Value * Time.deltaTime, ActiveBrushMode.Value);
